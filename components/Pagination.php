...
class Pagination
{
    ...
    /**
     * Для генерации HTML-кода канонических ссылок
     * @param integer $page - номер страницы
     * 
     * @return
     */
    public function getHtmlCanonical($page, $text = null)
    {
        $next = $page+1;
        $prev = $page-1;
        $limits = $this->limitsNext();
        $links = null;
        # Если текст ссылки не указан
        if (!$text)
        # Указываем, что текст - цифра страницы
        ...
        if($page == $limits[0]) {
            $links = '<link rel="canonical" '. $currentURI . $this->index . $page. '">';
            $links .= '<link rel="next" '. $currentURI . $this->index . $limits[1]. '">';
        } elseif ($page > $limits[0] && $page < $limits[1]) {
            $links = '<link rel="prev" '. $currentURI . $this->index . $limits[0]. '">';
            $links .= '<link rel="canonical" '. $currentURI . $this->index . $page. '">';
            $links .= '<link rel="next" '. $currentURI . $this->index . $limits[1]. '">';
        } elseif ($page == $this->amount) {
            $links = '<link rel="prev" '. $currentURI . $this->index . $prev. '">';
            $links .= '<link rel="canonical" '. $currentURI . $this->index . $page. '">';
        }
        return $links;
    }
    ...
    /**
     *  Для получения инфо, есть ли первая или последняя страница
     * 
     * @return массив с началом и концом отсчёта
     */
    private function limitsNext()
    {   
        $max = 2;
        # Вычисляем ссылки слева (чтобы активная ссылка была посередине)
        $left = $this->current_page - round($max / 2);
        
        # Вычисляем начало отсчёта
        $start = $left > 0 ? $left : 1;
        # Если впереди есть как минимум $this->max страниц
        if ($start + $max <= $this->amount) {
        # Назначаем конец цикла вперёд на $this->max страниц или просто на минимум
            ...
            $end = $this->amount;
            # Начало - минус $this->max от конца
            $start = $this->amount - $max > 0 ? $this->amount - $max : 1;
        }
        # Возвращаем
        return
                array($start, $end);
